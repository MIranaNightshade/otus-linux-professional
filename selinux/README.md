...
Можно было поменять контекст безопасности для каталога /etc/named например на named_zone_t, где процесс named_t имеет право на запись, но это может сломать работу процессов которые имеют права на файлы типа named_conf_t, созданный модуль же влияет только на процессы типа named_t (добавляет им права). 
